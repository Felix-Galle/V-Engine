...
if __name__ == '__main__':
    ...
    for arg in sys.argv:
        if arg.endswith('.v'):
            script_file = arg
        if arg == '--help':
            os.system("notepad.exe README.md") # Opens the README in notepad
        if arg == '--open':
            os.system("notepad.exe " + script_file)
        if arg == '--debug': # TODO: Fix, due to being called before creation
            os.system("notepad.exe " + log_file)
    ...
    logging.basicConfig(filename=log_file, level=logging.DEBUG, format='[%(name)s/%(levelname)s] %(asctime)s > %(message)s')
